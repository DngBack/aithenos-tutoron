...
class OpenAISearchService(LLMBaseService):
    ...
    def process(self, inputs: OpenAISearchInput) -> OpenAISearchOutput:
        """
        ...
        try:
            ...
            return OpenAISearchOutput(response=response)
        except Exception as e:
            logger.exception(
    ...
    def _inference_by_llm(
        ...
        if json_mode:
            return self.json_parse(response)  # type: ignore
